...
predict_BRF_class <- ifelse(predict_BRF >= 0.33, 1, 0)
...
pred_tune_BRF_class <- ifelse(pred_tune_BRF >= 0.33, 1, 0)
# Convert probability to factor
...
```{r}
# Load important packages for Logistic Regression
...
# Train Logistic Regression (LR) using the balanced(ADASYN) data 
LR_model1 <- glm(CHDRisk ~ ., data = train_ADASYN, family = "binomial")
summary(LR_model1)
...
LR_preds1 <- ifelse(LR_probs1 >= 0.33, "1", "0")
...
LR_feaures <- model.matrix(CHDRisk ~ ., data = train_ADASYN)[,-1] # extract features exclusing target
LR_target <- train_ADASYN$CHDRisk
# CV + HT
set.seed(123)
CV_LR <- cv.glmnet(
...
pred_Vclass <- ifelse(pred_Vprobs >= 0.3, 1, 0)
