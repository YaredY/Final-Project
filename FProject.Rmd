...
# Balanced Random Forest(BRF), Logistic Regression(LR), Support Vector Machine(SVM)
...
final_BRF_model <- ranger(CHDRisk ~ ., data = train_BALANCEDRF, num.trees = 500, 
                      ...
                      min.node.size = best1_minnz, # minimum number of samples in a terminal node
...
```{r}
# Model again to identify feature importance on prediction performance 
...
pred_Vclass <- ifelse(pred_Vprobs >= 0.3, 1, 0) # adjust classification threshold
...
```
# Step 4: Final Model Evaluation 
# Balanced Random Forest(BRF) using testing set
```{r}
# Step 4: Final Model Evaluation 
# Using testing set (unseen data)
# Balanced Random Forest(BRF)
# Predict and evaluate model using the testing set (unseen data)
pred_BRF_test <- predict(final_BRF_model, data = test_cleaned)$predictions[,"1"]
pred_BRF_test_class <- ifelse(pred_BRF_test >= 0.3, 1, 0) # adjust classification threshold
# Convert probability to factor
pred_BRF_test_class <- factor(pred_BRF_test_class, levels = c(0,1))
#  Calculate and print the Confusion Matrix and statistics (Recall, Precision, Balanced Accuracy......)
cat("Balanced Random Forest Model (BRF) Result", "\n")
cat("\n")
confusionMatrix(pred_BRF_test_class, test_cleaned$CHDRisk, positive = "1")
# Compute ROC(AUC) Area Under the Curve
probs_BRF_test <- pred_BRF_test
rocob_BRF_test <- roc(test_cleaned$CHDRisk, probs_BRF_test)
auc(rocob_BRF_test)
...
# Logistic Regression(LR)
```{r}
# Predict using testing set for final model evaluation
# Prepare the testing set
test_features <- model.matrix(CHDRisk ~ ., data = test_FADASYN)[,-1]
test_target <- test_FADASYN$CHDRisk
# Predict probabilities
pred_Tprobs <- predict(final_LRmodel, newx = test_features, type = "response")
# Convert probs to class 
pred_Tclass <- ifelse(pred_Tprobs >= 0.3, 1, 0) # adjust classification threshold
pred_Tclass <- factor(pred_Tclass, levels = c("0", "1")) # Make it a factor
# Calculate and print the Confusion Matrix and statistics (Recall, Precision, Balanced Accuracy......)
cat("Logistic Regression Model (LR) Result", "\n")
cat("\n")
confusionMatrix(pred_Tclass, test_target, positive= "1" )
# Calculate ROC(AUC) Area Under the Curve
roc_Tobj <- roc(test_target, as.numeric(pred_Tprobs))
auc(roc_Tobj)
...
```{r}
# Predict and evaluate the linear SVM  model using the testing set (unseen data)
pred_SVML_test <- attr(predict(final_LSVM_model, test_FADASYN, probability = TRUE), "probabilities") [,"1"]
# Probability class assigned 
prob_SVM_class_test <- ifelse(pred_SVML_test >= 0.3, 1, 0) # adjust classification threshold
# Change probability class into factor
prob_SVM_class_test <- factor(prob_SVM_class_test, levels = c("0", "1"))
# Calculate and print the Confusion Matrix and statistics (Recall, Precision, Balanced Accuracy......)
cat("Support Vector Machine (SVM) Result", "\n")
cat("\n")
confusionMatrix(prob_SVM_class_test, test_FADASYN$CHDRisk, positive = "1")
# ROC(AUC) Area under the curve 
roc_SVML_test <- roc(test_FADASYN$CHDRisk, pred_SVML_test)
auc(roc_SVML_test)
```
# ROC(AUC) Curve Analyis 
```{r}
library(ROCR)
```
...
```{r}
# Plot ROC (AUC)curve to compare models Area Under the Curve (AUC) values
# Prediction objects used for ROC curve analysis
# Predict values(probs) extracted from each models prediction
BRF_predict2 <- prediction(pred_BRF_test, test_cleaned$CHDRisk)
LR_predict2 <- prediction(pred_Tprobs, test_FADASYN$CHDRisk)
LSVM_predict2 <- prediction(pred_SVML_test, test_FADASYN$CHDRisk)

# ROC curves for Balanced Random Forest, Logistic Regression, and linear kernel SVM models
BRF_perform2 <- performance(BRF_predict2, "tpr", "fpr")
LR_perform2 <- performance(LR_predict2, "tpr", "fpr")
LSVM_perform2 <- performance(LSVM_predict2, "tpr", "fpr")

# Extract AUC(Area Under the Curve) values for comparison 
BRF_AUC2 <- round(performance(BRF_predict2, "auc")@y.values[[1]], 3)
LR_AUC2 <- round(performance(LR_predict2, "auc")@y.values[[1]], 3)
LSVM_AUC2 <- round(performance(LSVM_predict2, "auc")@y.values[[1]], 3)

#Plot the ROC curves with their corresponding AUC values
plot(BRF_perform2, col = "green", main = "ROC Curve Analysis")
plot(LR_perform2, col = "blue", add = TRUE)
plot(LSVM_perform2, col = "red", add = TRUE)

legend("bottomright", legend = c(paste("BRandom Forest (AUC = ", BRF_AUC2, ")", sep = ""),
                                 paste("Logistic Regression (AUC = ", LR_AUC2,")", sep =""), 
                                 paste("Linear SVM (AUC = ", LSVM_AUC2, ")", sep = "")),
       col = c("green", "blue", "red"), lwd = 2)

```

# Generate model comparison tibble
```{r}
# Models are compared based on important metrics for CHD prediction
# Recall, Balanced Accuracy, ROC(AUC), and Precision are the chosen metrics for comparison  
# Construct confusion matrix object from the confusion matrix result of each model above
BRF_CM <- confusionMatrix(pred_BRF_test_class, test_cleaned$CHDRisk, positive = "1")
LR_CM <- confusionMatrix(pred_Tclass, test_target, positive= "1" )
LSVM_CM <- confusionMatrix(prob_SVM_class_test, test_FADASYN$CHDRisk, positive = "1")

# Construct ROC(AUC) object from the ROC(AUC) result of each model above 
BRF_ROC <- auc(rocob_BRF_test)
LR_ROC <- auc(roc_Tobj)
LSVM_ROC <- auc(roc_SVML_test)
# Extract required metrics for comparison (function takes three arguments)
required_metrics <- function(Confusion_Matrix, ROC_AUC, ML_Model){
  
  # Extract only the important metrics from the confusionMatrics()
  recall_val <- Confusion_Matrix$byClass["Sensitivity"] # Sensitivity (which means recall) is extracted)
  precision_val <- Confusion_Matrix$byClass["Pos Pred Value"] # Pos Pred Value (which means Precision) is extracted
  balanced_acc <- Confusion_Matrix$byClass["Balanced Accuracy"] # Balanced Accuracy is extracted 
  auc_val <- ROC_AUC # AUC value is extracted 
  ...
  tibble(
    ...
    ROC_AUC = round(auc_val, 2)
  )
}
# Call the function to apply and extract the required metrics for each model
BRF_metrics <- required_metrics(BRF_CM, BRF_ROC, "Balanced Random Forest (BRF)")
LR_metrics <- required_metrics(LR_CM, LR_ROC, "Logistic Regression (LR)")
LSVM_metrics <- required_metrics(LSVM_CM, LSVM_ROC, "Linear Support Vector Machine (LSVM)")
# combine them into one tibble side by side for comparison 
compare_ML_models <- bind_rows(BRF_metrics, LR_metrics, LSVM_metrics) %>%
  arrange(desc(Recall)) # descending order by their Recall value 
# Print the comparison tibble
print(compare_ML_models)
# Looking at the output a model with a higher Recall value is selected as the best model for CHD prediction 
```
